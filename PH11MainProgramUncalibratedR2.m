...
%xzGrid = load('C:\Users\mpalmsten\Documents\projects\GOLDCOAST\data\preStormProfiles\MTA67059xz.dat');
xzGrid = load('C:\Users\mpalmsten\Documents\projects\GOLDCOAST\data\preStormProfiles\MTA63080xz.dat');
xTemp = xzGrid(:,1);
zTemp = xzGrid(:,2);
clear xzGrid;
...
% load final profile
%xzFinal = load('C:\Users\mpalmsten\Documents\projects\GOLDCOAST\data\postStormProfiles\MTA67060xz.dat');
xzFinal = load('C:\Users\mpalmsten\Documents\projects\GOLDCOAST\data\postStormProfiles\MTA63081xz.dat');
zTemp = xzFinal(:,2);
xTemp = xzFinal(:,1);
keep = find(zTemp>=-1);
zFinal = zTemp(keep);
...
WaveData = load('C:\Users\mpalmsten\Documents\projects\GOLDCOAST\data\stormHydrograph\waveConditionsGoldCoast.txt');
WLData = load('C:\Users\mpalmsten\Documents\projects\GOLDCOAST\data\stormHydrograph\waterLevelGoldCoast.txt');
t1 = WLData(:,1);
dt1 = diff(t1(1:2));
WL = WLData(:,2);
Hs = WaveData(:,1);
...
dVResidual(tt) = Vc(ii)-dVT(tt);
 ...
 % final retreat
 xF = interp1(zFinal(find(isnan(zFinal)~=1)),xM(find(isnan(zFinal)~=1)),zF)
 x0 = interp1(z,xM,0);
 deltaX = xToe(end)-x0;
 deltaZ = zb(end)-0;
%  figure
